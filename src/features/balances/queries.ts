...
export async function getBalanceSummaries(): Promise<AccountBalanceSummary[]> {
  try {
    ...
    // Fetch raw USD spend via AdAccount.accountId with fallback to integration_settings bridge
    ...
    if (accountIds.length > 0) {
      try {
        ...
        console.error("[getBalanceSummaries] Spend query failed:", spendErr);
      }
    }

    return accounts.map((acct) => {
      const totalTopUp = topUpMap.get(acct.id) ?? 0;
      const totalSpent = spendMap[acct.id] ?? 0;
      return {
        accountId:   acct.id,
