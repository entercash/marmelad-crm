...
export async function syncAllTaboolaCampaigns(): Promise<SyncTaboolaResult> {
  ...
  // ── Step 5: Enrich publisher GEO from Keitaro ──────────────────────────
  // Taboola API doesn't provide country per publisher. Keitaro sub_id_6 = {country}.
  // Query Keitaro grouped by sub_id_4 (site_id) + sub_id_6 (country) and
  // update publisher_stats_daily.geo where it's "XX".
  try {
    const { getKeitaroSettings } = await import("@/features/integration-settings/queries");
    const { KeitaroClient } = await import("@/integrations/keitaro/client");

    const kSettings = await getKeitaroSettings();
    if (kSettings.apiUrl && kSettings.apiKey) {
      const kClient = new KeitaroClient({ apiUrl: kSettings.apiUrl, apiKey: kSettings.apiKey });

      // Get all linked Keitaro campaign IDs
      const campaignLinks = await prisma.campaignLink.findMany({
        select: { keitaroCampaign: { select: { externalId: true } } },
      });
      const keitaroIds = Array.from(new Set(campaignLinks.map((l) => l.keitaroCampaign.externalId)));

      if (keitaroIds.length > 0) {
        const report = await kClient.buildReport({
          range: { from: startDate, to: endDate, timezone: "Europe/London" },
          grouping: ["sub_id_4", "sub_id_6", "day"],
          metrics: ["clicks"],
          limit: 100_000,
          offset: 0,
        });

        // Build site_id → day → country (pick country with most clicks)
        const siteGeo = new Map<string, Map<string, { country: string; clicks: number }>>();
        for (const row of report.rows) {
          const siteId = String(row.sub_id_4 ?? "").trim();
          const country = String(row.sub_id_6 ?? "").trim().toUpperCase();
          const day = String(row.day ?? "").trim();
          const clicks = Number(row.clicks ?? 0);
          if (!siteId || !country || !/^[A-Z]{2}$/.test(country) || !day) continue;

          if (!siteGeo.has(siteId)) siteGeo.set(siteId, new Map());
          const dayMap = siteGeo.get(siteId)!;
          const existing = dayMap.get(day);
          if (!existing || clicks > existing.clicks) {
            dayMap.set(day, { country, clicks });
          }
        }

        // Batch update publisher_stats_daily.geo
        let geoUpdated = 0;
        for (const [siteId, dayMap] of Array.from(siteGeo.entries())) {
          // Find publisher by externalId
          const pub = await prisma.publisher.findFirst({
            where: { externalId: siteId },
            select: { id: true },
          });
          if (!pub) continue;

          for (const [day, { country }] of Array.from(dayMap.entries())) {
            const date = fromApiDate(day);
            // Update all XX rows for this publisher+day to the detected country
            const result = await prisma.publisherStatsDaily.updateMany({
              where: {
                publisherId: pub.id,
                date,
                geo: "XX",
            ...
            geoUpdated += result.count;
          }
        }

        console.log(`[taboola:geo] Enriched ${geoUpdated} publisher_stats_daily rows with Keitaro GEO`);
      }
    }
  } catch (geoErr) {
    console.error("[taboola:geo] GEO enrichment failed (non-fatal):", geoErr);
  }
